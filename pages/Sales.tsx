...
const Sales: React.FC = () => {
  ...
  return (
    <div className="space-y-6 md:space-y-12 pb-32 max-w-7xl mx-auto animate-in fade-in duration-700">
      <header className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 md:gap-8 bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[4rem] border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2 max-w-2xl">
            <div className="flex items-center gap-2 text-brand-600 mb-2">
                ...
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em]">Console de Distribution</span>
            </div>
            <h1 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tight leading-tight">Vendre un Ticket</h1>
            <p className="text-slate-400 font-medium text-xs md:text-lg">Sélectionnez l'offre adaptée au besoin du client.</p>
        </div>
        {userProfile?.role === UserRole.REVENDEUR && (
            <div className="w-full xl:w-auto bg-slate-900 text-white p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl relative z-10 flex items-center gap-4 md:gap-5 border border-white/10 group overflow-hidden">
                <div className="absolute inset-0 bg-brand-600 opacity-0 group-hover:opacity-10 transition-opacity"></div>
                <div className="w-10 h-10 md:w-14 md:h-14 bg-brand-600 rounded-xl md:rounded-2xl flex items-center justify-center text-white shadow-lg shadow-brand-500/30 group-hover:scale-110 transition-transform shrink-0">
                    <Wallet className="w-5 h-5 md:w-7 md:h-7" />
                </div>
                <div>
                    <p className="text-[9px] md:text-[10px] font-black text-brand-400 uppercase tracking-widest leading-none mb-1 md:mb-1.5">Mon Solde Actuel</p>
                    <p className="text-xl md:text-3xl font-black tracking-tight">{(userProfile.balance || 0).toLocaleString()} <span className="text-[9px] md:text-[10px] text-brand-400 ml-1">{currency}</span></p>
                </div>
            </div>
        )}
      </header>
      ...
      <div className="relative max-w-2xl mx-auto group px-2">
          ...
          <Search className="absolute left-5 md:left-6 top-1/2 -translate-y-1/2 w-5 h-5 md:w-6 md:h-6 text-slate-300 group-focus-within:text-brand-600 transition-colors" />
          <input type="text" placeholder="Rechercher (ex: 24H, 1GB...)" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 md:pl-16 pr-6 py-4 md:py-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-100 bg-white shadow-sm focus:ring-4 md:focus:ring-[12px] focus:ring-brand-50 outline-none font-bold text-slate-700 transition-all text-base md:text-xl" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 px-2">
        {loading ? (
          ...
          <div key={p.id} className="bg-white p-5 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between min-h-[14rem] md:h-[22rem] group relative overflow-hidden">
             ...
             <div>
                 <div className="flex justify-between items-start mb-4 md:mb-6">
                     <div className="w-10 h-10 md:w-14 md:h-14 bg-brand-50 text-brand-600 rounded-xl md:rounded-2xl flex items-center justify-center group-hover:bg-brand-600 group-hover:text-white transition-all shadow-sm">
                         <Tag className="w-5 h-5 md:w-7 md:h-7" />
                     </div>
                     <div className="text-right">
                         <p className="text-xl md:text-3xl font-black text-slate-900 tracking-tight">{p.price.toLocaleString()} <span className="text-[9px] md:text-[10px] uppercase font-bold text-slate-400">{currency}</span></p>
                         <p className="text-[8px] md:text-[9px] font-black text-brand-500 uppercase tracking-widest mt-1">Prix de vente</p>
                     </div>
                 </div>
                 <h3 className="text-lg md:text-2xl font-black mb-1.5 text-slate-900 group-hover:text-brand-600 transition-colors tracking-tight line-clamp-2">{p.name}</h3>
                 <div className="flex items-center gap-2">
                     <div className={`w-2 h-2 rounded-full ${p.tickets?.[0]?.count < 20 ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}></div>
                     <p className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest ${p.tickets?.[0]?.count < 20 ? 'text-red-500' : 'text-slate-400'}`}>{p.tickets?.[0]?.count || 0} Tickets Dispo</p>
                 </div>
             </div>
             
             <button onClick={() => { setSelectedProfile(p); setShowPhoneModal(true); }} disabled={!p.tickets?.[0]?.count} className={`w-full py-4 md:py-5 rounded-[1.5rem] md:rounded-[2rem] font-black text-[10px] md:text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all transform active:scale-95 shadow-xl mt-4 md:mt-6 ${!p.tickets?.[0]?.count ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-slate-900 hover:bg-black text-white hover:shadow-slate-200'}`}>
                 VENDRE <ArrowRight className="w-4 h-4 md:w-5 md:h-5" />
             </button>
          </div>
        )) : (
      ...
      {showPhoneModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] p-6 md:p-10 relative z-10 shadow-2xl overflow-hidden border border-slate-100">
      ...
      {showSuccessModal && lastSoldTicket && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 animate-in fade-in">
          ...
          <div className="bg-white w-full max-w-md max-h-[90vh] overflow-y-auto rounded-[3rem] p-6 md:p-10 relative z-10 text-center shadow-2xl animate-in zoom-in-95 duration-500 custom-scrollbar">
