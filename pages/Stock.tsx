...
const Stock: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (user) {
        const { data: userData } = await db.from('users').select('role, tenant_id').eq('id', user.id).single();
        if (userData) {
  ...
  const fetchStock = async () => {
    ...
    try {
      ...
      if (currentUser.role === UserRole.ADMIN_GLOBAL) { if (agencyFilter !== 'ALL') query = query.eq('tenant_id', agencyFilter); } else { query = query.eq('tenant_id', currentUser.tenant_id); }
  ...
  const handleVerifyDelete = (ticket: any) => { if (currentUser?.role === UserRole.REVENDEUR) { showToast('error', "Action refusée."); return; } if (ticket.status !== 'NEUF') { showToast('error', "Impossible de supprimer un ticket vendu."); return; } setTicketToDelete(ticket); };
  const performDelete = async () => { if (!ticketToDelete) return; setIsDeleting(true); try { const { error: dError } = await db.from('tickets').delete().eq('id', ticketToDelete.id); if (dError) throw dError; setTickets(prev => prev.filter(t => t.id !== ticketToDelete.id)); showToast('success', `Ticket supprimé.`); setTicketToDelete(null); } catch (err: any) { showToast('error', err.message); } finally { setIsDeleting(false); } };
